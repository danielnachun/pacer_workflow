...
find_vcfs() {
    input_directory=$1
    vcf_list_directory=$2
    n_vcfs=$3

    mkdir -p ${vcf_list_directory}/${n_vcfs}

    chrs=$(bcftools index -s $(find -L ${input_directory} -type f -name '*.vcf.gz' | grep -v 'chr' | sort -u | head -n 1) | cut -f 1)
    echo ${chrs} | tr ' ' '\n' | xargs --replace=% bash -c "find -L ${input_directory} -type f -name '*%.vcf.gz' | sort -u | head -n ${n_vcfs} > ${vcf_list_directory}/${n_vcfs}/%_vcfs"
}

export -f find_vcfs

# echo 50 | tr ' ' '\n' | xargs --replace=% bash -c "find_vcfs ${input_directory} ${vcf_list_directory} %"
find_vcfs ${input_directory} ${vcf_list_directory} 8000

vcf_array_length=$(ls ${vcf_list_directory} | wc -l) #get the number of FASTQs

vcf_lists=$(dirname ${input_directory})/vcf_lists_saturation
find -L "${vcf_list_directory}" -type f `#list all files in ${vcf_directory}` | \
    sort -u > ${vcf_lists}
vcf_array_length=$(wc -l < ${vcf_lists}) #get the number of FASTQs
